...
YoLoV6::YoLoV6(std::shared_ptr<BMNNContext> context) : m_bmContext(context) {
    std::cout << "YoLoV6 ctor .." << std::endl;
}

YoLoV6::~YoLoV6() {
    std::cout << "YoLoV6 dtor ..." << std::endl;
...
int YoLoV6::Init(float confThresh, float nmsThresh, const std::string& coco_names_file, const std::vector<float>& confs) {
...
void YoLoV6::enableProfile(TimeStamp* ts) {
    m_ts = ts;
}

int YoLoV6::batch_size() {
    return max_batch;
};

int YoLoV6::Detect(const std::vector<bm_image>& input_images, std::vector<YoLoV6BoxVec>& boxes) {
    int ret = 0;
    // 3. preprocess
    m_ts->save("YoLoV6 preprocess", input_images.size());
    // auto start = std::chrono::high_resolution_clock::now();
    ret = pre_process(input_images);
    // std::cout << "pre_process: " <<(std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - start).count()) /1000.0 << std::fixed << std::setprecision(3) <<"ms" << std::endl;
    CV_Assert(ret == 0);
    m_ts->save("YoLoV6 preprocess", input_images.size());

    // 4. forward
    m_ts->save("YoLoV6 inference", input_images.size());
    ret = m_bmNetwork->forward();
    CV_Assert(ret == 0);
    m_ts->save("YoLoV6 inference", input_images.size());

    // 5. post process
    m_ts->save("YoLoV6 postprocess", input_images.size());
    if(use_cpu_opt){
#if USE_OPT
...
int YoLoV6::pre_process(const std::vector<bm_image>& images) {
...
float YoLoV6::get_aspect_scaled_ratio(int src_w, int src_h, int dst_w, int dst_h, bool* pIsAligWidth) {
    float ratio;
...
int YoLoV6::post_process(const std::vector<bm_image>& images, std::vector<YoLoV6BoxVec>& detected_boxes) {
    YoLoV6BoxVec yolobox_vec;
...
#if USE_OPT
int YoLoV6::post_process_opt(const std::vector<bm_image>& images, std::vector<YoLoV6BoxVec>& detected_boxes) {
    // std::cout << "post_opt$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$" << std::endl;
    YoLoV6BoxVec yolobox_vec;
    std::vector<cv::Rect> bbox_vec;
    std::vector<std::shared_ptr<BMNNTensor>> outputTensors(output_num);
    ...
    for (int batch_idx = 0; batch_idx < images.size(); ++batch_idx) {
        ...
        output_data = (float*)out_tensor->get_cpu_data() + batch_idx * feat_num * (m_class_num + mask_num + 4); //每帧的输出张量指针
        LOG_TS(m_ts, "post 1: get output");

        // Candidates
        LOG_TS(m_ts, "post 2: get detections matrix nx6 (xyxy, conf, cls)");
        float* cls_conf = output_data + 4 + 1; // v6检测头：4+1+m_class_num
        for (int i = 0; i < feat_num; i++) {
#if USE_MULTICLASS_NMS
            // multilabel
            for (int j = 0; j < m_class_num; j++) {
            ...
            // LOG_TS(m_ts, "post3");
            // 按每个类置信度做过滤
            if (max_value >= m_confThreshold && max_value >= conf_thresholds[max_index]) {
                YoLoV6Box box;
                box.score = max_value;
                box.class_id = max_index;
                // 对不同类别的框设置相应位置偏移，防止nms时不同类别的框重叠或相互干扰
                int c = box.class_id * max_wh;
                // 从模型输出提取检测框信息
                float centerX = output_data[i* nout ];
                float centerY = output_data[i * nout + 1 ];
...
void YoLoV6::clip_boxes(YoLoV6BoxVec& yolobox_vec, int src_w, int src_h) {
    for (int i = 0; i < yolobox_vec.size(); i++) {
...
void YoLoV6::xywh2xyxy(YoLoV6BoxVec& xyxyboxes, std::vector<std::vector<float>> box) {
...
void YoLoV6::NMS(YoLoV6BoxVec& dets, float nmsConfidence) {
    int length = dets.size();
    int index = length - 1;
    
    //升序排列
    std::sort(dets.begin(), dets.end(), [](const YoLoV6Box& a, const YoLoV6Box& b) { return a.score < b.score; });
...
void YoLoV6::draw_bmcv(bm_handle_t& handle,
                       int classId,
                       float conf,
                       int left,
                       int top,
